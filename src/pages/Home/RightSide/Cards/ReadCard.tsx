const ReadCard = () => {
  return (
    <section className="w-full h-[16vh] overflow-hidden border-[1px] border-neutral-200 shadow relative rounded-lg">
      <section className="w-full h-[40vh] overflow-hidden relative">
        <img
          src={"/images/landing7.png"}
          style={{ width: "100%", height: "100%" }}
          className="object-cover w-full"
          alt="Image"
        />
        <div className={"absolute inset-0 bg-black opacity-30"}></div>{" "}
        {/* This div acts as the overlay */}
      </section>

      <section
        style={{ width: "100%", height: "100%", top: 0, left: 0 }}
        className="absolute object-cover w-full items-center justify-center flex flex-col gap-4 z-1"
      >
        <div
          onClick={() => alert("Coming soon!")}
          className="cursor-pointer text-white flex flex-col items-center text-center gap-2"
        >
          <span className="text-[15pt] text-gray-50 font-extrabold">
            Read out latest whitepapers
          </span>
        </div>
      </section>
    </section>
  );
};

export default ReadCard;
